...
import $ from './method'
import { TYPESET, UNICODE } from './regex'
...
export class Finder extends Fibre {
  ...
  groupify(option) {
    ...
    if (option.biaodian) {
      this.replace(
        TYPESET.group.biaodian[0],
        createBDGroup,
      ).replace(
        TYPESET.group.biaodian[1],
        createBDGroup,
      )
    }
    ...
    if (option.hanzi || option.cjk) {
      this.wrap(
        TYPESET.group.hanzi,
        $.clone($.create('h-char-group', 'hanzi cjk')),
      )
    }
    if (option.western) {
      this.wrap(
        TYPESET.group.western,
        $.clone($.create('h-word', 'western')),
      )
    }
    if (option.kana) {
      this.wrap(
        TYPESET.group.kana,
        $.clone($.create('h-char-group', 'kana')),
      )
    }
    if (option.eonmun || option.hangul) {
      this.wrap(
        TYPESET.group.eonmun,
        $.clone($.create('h-word', 'eonmun hangul')),
      )
    }
